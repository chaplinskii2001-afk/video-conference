...
class ModelManager:
    ...
    def _validate_model_dirs(self) -> None:
        ...
        paths = {
            "whisper_path": model_cfg.get("whisper_path"),
            "diarization_path": model_cfg.get("diarization_path"),
            "qwen_path": model_cfg.get("qwen_path"),
        }
        for k, p in paths.items():
            if not p or not os.path.exists(p):
                raise FileNotFoundError(f"Model dir missing: {k}={p!r}")
    ...
    def whisper_transcribe(self, audio_path: str) -> Dict:
        """
        Транскрипция с анти-галлюцинационными параметрами.
        
        Параметры для подавления галлюцинаций:
        - no_speech_threshold=0.4: порог детекции не-речи
        - logprob_threshold=-0.8: минимальная уверенность модели
        - compression_ratio_threshold=2.4: детекция повторов
        - condition_on_previous_text=False: отключить "память" между VAD-сегментами
        """
        ...
        # Базовые параметры. В transformers generate() temperature = float (не list),
        # поэтому fallback по температурам делаем вручную.
        ...
        def _score_result(result: Dict) -> float:
            ...
            # Сильная повторяемость = высокий compression ratio => штрафуем
            penalty = 0.0
            if cr > 2.4:
                penalty += (cr - 2.4) * 200.0
            return float(len(text)) - penalty

        try:
            # Оставляем только аргументы, которые реально принимает model.generate(),
            # чтобы не падать на неожиданных kwargs при смене версии transformers.
            gen_sig = inspect.signature(self.whisper_pipeline.model.generate)
            ...
            for t in temperature_schedule:
                ...
                generate_kwargs["temperature"] = float(t)

                safe_generate_kwargs = {
                    k: v for k, v in generate_kwargs.items() if k in allowed
                }

                res = self.whisper_pipeline(
                    audio_path,
                    batch_size=self.whisper_batch_size,
                    generate_kwargs=safe_generate_kwargs,
                    return_timestamps=True,
                )
                ...
                # Быстрый выход: на t=0 обычно самый стабильный результат,
                # если текст осмысленный и не супер-повторяющийся.
                if t == 0.0 and best_score > 30:
                    break

            return best if best is not None else {"text": "", "chunks": []}
            
        except TypeError:
            # Fallback для старых версий transformers
            self.logger.warning("generate_kwargs не поддерживается, используем базовые параметры")
    ...
    async def load_diarization(self, skip_unload: bool = False) -> bool:
        ...
        try:
            ...
            self.gpu_manager.take_snapshot("after_diarization_load")
