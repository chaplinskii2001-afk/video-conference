...
async def lifespan(app: FastAPI):
    # Startup
    logger.info(f"Запуск приложения... ({_get_tomsk_time().strftime('%Y-%m-%d %H:%M:%S')})")
    ...
    model_dirs = {
        ...
        "xdg_cache": os.getenv("XDG_CACHE_HOME", "/app/models/xdg_cache"),
        "torch_home": os.getenv("TORCH_HOME", "/app/models/torch_home"),
    }
    ...
    # Shutdown
    logger.info(f"Остановка приложения... ({_get_tomsk_time().strftime('%Y-%m-%d %H:%M:%S')})")
    if processor:
